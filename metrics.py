...
class MetricsSnapshot:
    ...
    __slots__ = ("frame", "psnr", "l1", "iou",
                 "ssim_xy", "ssim_xz", "ssim_yz", "ssim_mean")
...
class MetricsCollector:
    ...
    def _compile_pipelines(self):
        ...
        prog_clear = self.device.load_program("shaders/metrics.slang", ["clear_metrics"])
    ...
    def _run_bin_pass(self, vol_min: tuple, vol_max: tuple):
        ...
        with cmd.begin_compute_pass() as cp:
            ...
            cursor["TrainParams"]["tileSize"]       = r.tile_size  # expose as attribute if needed
    ...
    def _dispatch_ssim(self, cmd, vol_min, vol_max):
        ...
        axes = [
            (0, self.vol_res[2], self.vol_res[0], self.vol_res[1]),  # XY slices, iterate Z
            (1, self.vol_res[1], self.vol_res[0], self.vol_res[2]),  # XZ slices, iterate Y
            (2, self.vol_res[0], self.vol_res[1], self.vol_res[2]),  # YZ slices, iterate X
        ]
    ...
    def _readback(self, frame: int, has_ssim: bool):
        ...
        if has_ssim and self.config.enable_ssim:
            ...
            self._last_snap.ssim_mean = (self._last_snap.ssim_xy +
                                        self._last_snap.ssim_xz +
                                        self._last_snap.ssim_yz) / 3.0
    ...
    def export_csv(self, path: str):
        ...
        with open(path, "w", newline="") as f:
            ...
            for row in zip(self._frames, self._psnr,
                           self._l1, self._iou, self._ssim_mean):
                w.writerow([
    ...
    def draw_ui_inline(self):
        ...
        # Enable toggles
        _, cfg.enable_psnr = imgui.checkbox("PSNR##me",  cfg.enable_psnr); imgui.same_line()
        _, cfg.enable_l1   = imgui.checkbox("L1##me",    cfg.enable_l1);   imgui.same_line()
        _, cfg.enable_iou  = imgui.checkbox("IoU##me",   cfg.enable_iou);  imgui.same_line()
        ...
        def metric_row(label, val, good_dir, green_t, red_t):
            ...
            if np.isnan(val):
                ...
                return
            t = ((val - red_t) / (green_t - red_t + 1e-9) if good_dir == "high"
        ...
        def safe_plot(label, data, uid):
            ...
            if len(clean) < 2:
                imgui.text_colored(imgui.ImVec4(0.4,0.4,0.4,1), f"{label}: waiting...")
                return
            arr = np.array(clean, dtype=np.float32)
            lo, hi = float(arr.min()), float(arr.max())
            if hi - lo < 1e-9: hi = lo + 1e-3
            imgui.text(label)
            imgui.plot_lines(f"##mp{uid}", arr,
